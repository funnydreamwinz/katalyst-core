...
func (ha *HeadroomAssemblerCommon) getHeadroomByUtil() (resource.Quantity, map[int]resource.Quantity, error) {
	...
	bindingNUMAs, nonBindingNumas, err := ha.getReclaimNUMABindingTopo()
	if err != nil {
	...
	if len(nonBindingNumas) > 0 {
		cpuSet, ok := reclaimPoolInfo.TopologyAwareAssignments[commonstate.FakedNUMAID]
		if !ok {
			return resource.Quantity{}, nil, fmt.Errorf("reclaim pool NOT found TopologyAwareAssignments with fake numa")
		}

		reclaimMetrics, err := metricHelper.GetReclaimMetrics(cpuSet, ha.getReclaimCgroupPath(), ha.metaServer.MetricsFetcher)
		if err != nil {
			return resource.Quantity{}, nil, fmt.Errorf("get reclaim Metrics failed: %v", err)
		}

		options.MaxCapacity = dynamicConfig.MaxHeadroomCapacityRate * float64(ha.metaServer.MachineInfo.NumCores)
		headroom, err := ha.getUtilBasedHeadroom(options, reclaimMetrics)
		if err != nil {
